...
class CanprogWorker(QThread):
    """Integrated CANPROG worker.

    This replaces the old subprocess/canprog.exe method.
    It calls the existing canprog logic directly, using the same
    canprog modules, protocol class, FileManager, erase/write/verify/go,
    and 125 kbit/s IXXAT bootloader configuration.
    """
    ...
    def run(self):
        ...
        try:
            ...
            if self.operation == "read_info":
                # Read a small flash area only to force connection/read and display
                # bootloader version/chip ID from the existing CANPROG connect logic.
...
class STM32CanProgGUI(QWidget):
    ...
    def read_info(self):
        ...
        self.worker = CanprogWorker(operation="read_info", read_size=0x10)
    ...
    def run_full_update(self):
        ...
        self.console.append("Using integrated CANPROG engine.")
        self.console.append(f"Firmware: {hex_file}")
